...
def check_stderroutput(*popenargs, **kwargs):
    ...
    * It does omitting `None` in *cmd.
    ...
    if cmd is None:
        cmd = popenargs[0]
    #
    process = subprocess.Popen(_filter_args(cmd), stderr=subprocess.PIPE, **kwargs)
    stdout_output, stderr_output = process.communicate()
    retcode = process.poll()
    if retcode:
        raise subprocess.CalledProcessError(retcode, list(cmd), output=stderr_output)
...
def duration_to_hhmmss(*durations):
    ...
    def _conv(n):
        ...
        d -= ss_m * 60
        ss_s = int(d)
        return "%s%02d:%02d:%02d.%s" % ("" if n >= 0 else "-", ss_h, ss_m, ss_s, frac)
...
def read_audio(audio_file):
    ...
    rate, data = scipy.io.wavfile.read(audio_file, mmap=True)
    return data, rate
...
def _parse_ffprobe_output(inputstr):
    ...
    def _split_csv(s):
        ...
        while i < len(ss):
            result.append(ss[i])
            while i < len(ss) - 1 and result[-1].count("(") != result[-1].count(")"):
                i += 1
    ...
    for line in lines:
        ...
        if strmtype == "Video":
            spl = _split_csv(rest)
            resol = list(
                filter(lambda item: re.search(r"[1-9]\d*x[1-9]\d*", item), spl)
            )[0]
...
def _summarize_streams(streams):
    ...
    for st in streams:
        if st["type"] == "Video":
            ...
            result["max_resol_width"] = max(result["max_resol_width"], new_w)
            result["max_resol_height"] = max(result["max_resol_height"], new_h)
            if "fps" in st:
                result["max_fps"] = max(result["max_fps"], st["fps"])
        elif st["type"] == "Audio":
...
def media_to_mono_wave(
    ...
    if starttime_offset > 0:
        _ss_args = ("-ss", duration_to_hhmmss(starttime_offset))
...
def call_ffmpeg_with_filtercomplex(
    ...
    ifile_args = chain.from_iterable([("-i", _pathconv(f)) for f in inputfiles])
    #
    if vmap:
    ...
    try:
        ...
        buf = sys.stdout
    if mode == "script_bash":
        print("USIDNFALISDJFLSJDFÖLKAJSDLFKJASLDKFJLASJÖD")
        _quote = pipes_quote()
        buf.write(
            """\
#! /bin/sh
...
""".format(
        ...
        tempfn = os.path.join(os.environ.get("TEMP", "/tmp"), d.hexdigest() + ".txt")
        # print(f"Temporary file will be created at: {tempfn}")
...
try:
    ...
    try:
        ...
        pass
""".format(
                    filter_complex, json.dumps(tempfn), cmdstr
                ).encode(
                    "utf-8"
                )
            )
            ...
            with io.open(tempfn, "w") as fo:
                fo.write(filter_complex)
                # print(f"Temporary file successfully created at: {tempfn}")
            try:
                check_call(cmd)
            finally:
                try:
