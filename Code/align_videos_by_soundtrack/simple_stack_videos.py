    ...
    def build_stack_videos(self, ivmaps):
        ...
        if self._shape[0] > 1:
            for i in range(self._shape[1]):
                ...
                inputs = len(ivmaps[i * self._shape[0] : (i + 1) * self._shape[0]])
                fhstack.add_filter("hstack", inputs=inputs, shortest="1")
        ...
        if self._shape[1] > 1:
            # vstack
            fvstack.add_filter("vstack", inputs=self._shape[1], shortest="1")
            fvstack.ov.append("[v]")
...
def _build(args):
    ...
    files = check_and_decode_filenames(args.files, min_num_files=2, exit_if_error=True)
    if len(files) < shape[0] * shape[1]:
        files = files + [
            files[i % len(files)] for i in range(shape[0] * shape[1] - len(files))
        ]
    else:
    ...
    #
    with SyncDetector(
        params=args.summarizer_params, clear_cache=args.clear_cache
    ) as det:

        ares = det.align(files, known_delay_map=args.known_delay_map)
    ...
    for i, inf in enumerate(ares):
        ...
        vf = ",".join(
            filter(None, [v_filter_extra.get(""), v_filter_extra.get("%d" % i)])
        )
        af = ",".join(
            filter(None, [a_filter_extra.get(""), a_filter_extra.get("%d" % i)])
        )
...
def main(args=sys.argv):
    parser = cli_common.AvstArgumentParser(
        description="""\
...
different thing from this. See the option description."""
    ...
    parser.add_argument(
        "files", nargs="+", help="The media files which contains both video and audio."
    )
    ...
    parser.add_argument(
        "--audio_mode",
        choices=["amerge", "multi_streams", "individual"],
        default="amerge",
        help="""\
...
implies --video_mode='individual'. (default: %(default)s)""",
    ...
    parser.add_argument(
        "--video_mode",
        choices=["stack", "multi_streams", "individual"],
        default="stack",
        help="""\
...
implies --audio_mode='individual'. (default: %(default)s)""",
    ...
    if args.video_mode == "individual" or args.audio_mode == "individual":
        ...
        outfiles = ["{}_{:02d}{}".format(outbase, i, outext) for i in range(len(vmap))]
    else:
