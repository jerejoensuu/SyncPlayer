...
def get_video_offsets(video1, video2, params=None):
    """
    Get the offsets of the videos in seconds.
    """
    with SyncDetector(params=params) as sd:
        einf = sd.align([video1, video2])
    return [float("%.3f" % inf["trim"]) for inf in einf]
...
def build(args):
    ...
    with SyncDetector(
        ...
        einf = sd.align(files, known_delay_map=args.known_delay_map)
...
Create a simultaneous playing player using the video and audio elements of html 5."""
    ...
    # print(build(args))
    with open(
        r"C:\Users\jerep\Python projects\align-videos-by-sound\build\lib\test.html", "w"
    ) as f:
        f.write(build(args))
